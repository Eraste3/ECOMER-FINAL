  ...
  if (!lot) {
    return (
      <div className="p-8 text-center">
        ...
        <Button className="mt-4" onClick={() => navigate('/ecoshop')}>Retour à la marketplace</Button>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-4xl space-y-6 px-4 pb-12 sm:px-0">
      ...
      <div className="overflow-hidden rounded-2xl bg-white ring-1 ring-hairline shadow-card">
        <div className="aspect-video w-full bg-slate-100 sm:aspect-[21/9]">
          <img src={lot.images[0]} alt={lot.id} className="h-full w-full object-cover" />
        </div>
        ...
        <div className="p-6 sm:p-8">
          <div className="flex flex-wrap items-start justify-between gap-6">
            <div>
              <div className="flex items-center gap-3">
                <h1 className="font-display text-2xl font-bold text-navy">{lot.id}</h1>
            ...
            <div className="text-right">
              <p className="font-display text-3xl font-bold text-ocean">{totalPrice.toLocaleString()} €</p>
              <p className="text-sm text-slate-500">{lot.pricePerTon} € / tonne</p>
            </div>
          </div>
          ...
          <div className="mt-8 grid gap-8 sm:grid-cols-2">
            <div>
              ...
              <ul className="mt-6 space-y-3">
                <li className="flex items-center gap-3 text-sm text-slate-600">
                  ...
                  Quantité totale : <strong className="text-navy">{lot.quantityTons} tonnes</strong>
                </li>
                <li className="flex items-center gap-3 text-sm text-slate-600">
                  <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-surface text-ocean">
                    <CheckCircle2Icon className="h-4 w-4" />
                  </div>
                  Tri effectué par les équipes ECOMER
                </li>
              </ul>
            </div>
            ...
            <div className="rounded-xl bg-surface p-6 ring-1 ring-inset ring-hairline">
              ...
              <p className="mt-1 text-[11px] text-slate-500">Le paiement se fera par facturation hors ligne après validation.</p>
