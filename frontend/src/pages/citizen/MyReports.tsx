...
export function MyReportsPage() {
  const { getMyReports } = useSignalements();
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        ...
        setReports(data);
        if (data.length > 0) {
          setOpen(data[0].id?.toString() || null);
        }
      } catch (error) {
  ...
  const shown = filter === 'tous' ? reports : reports.filter((r) => r.status === filter);
    ...
    <div className="space-y-4 px-4 py-5 sm:px-0">
      <div>
        ...
        <p className="mt-0.5 text-xs text-slate-500">
          Suivez l'avancement de chaque signalement.
        </p>
      </div>
      ...
      <ul className="space-y-3">
          ...
          const currentIndex = timeline.indexOf(status as ReportStatus);
          const expanded = open === r.id?.toString();
          return (
            <li key={r.id} className="overflow-hidden rounded-xl bg-white ring-1 ring-hairline shadow-card">
                <button
                onClick={() => setOpen(expanded ? null : r.id?.toString() || null)}
                ...
                <span
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-[11px] text-slate-500">
                      {wasteMeta[wasteType as keyof typeof wasteMeta]?.label || wasteType}
                    </p>
                  </div>
                  <div className="flex shrink-0 items-center gap-2">
                    <StatusBadge
                    label={reportStatusMeta[status as ReportStatus]?.label || status}
                    tone={reportStatusMeta[status as ReportStatus]?.tone || 'neutral'} />
                    <ChevronDownIcon
                    ...
                    <div className="space-y-5 p-4">
                      ...
                      <dl className="grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">
                        <div className="rounded-lg bg-surface p-3 ring-1 ring-hairline">
                          ...
                          <dd className="mt-0.5 font-semibold text-navy">{wasteMeta[wasteType as keyof typeof wasteMeta]?.short || wasteType}</dd>
                        </div>
                        <div className="rounded-lg bg-surface p-3 ring-1 ring-hairline">
                          <dt className="text-[10px] uppercase tracking-wide text-slate-500">Gravité</dt>
                          <dd className="mt-1">
                            <SeverityBadge severity={r.gravite || 'moyenne'} />
                          </dd>
                        </div>
                        <div className="rounded-lg bg-surface p-3 ring-1 ring-hairline">
                        ...
                        <div className="rounded-lg bg-surface p-3 ring-1 ring-hairline">
                          ...
                          <dd className="mt-0.5 font-semibold text-navy">{r.description || 'N/A'}</dd>
                        </div>
                      </dl>
                    </div>
                  </motion.div>
